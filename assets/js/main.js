  ...
  (function initPlanet() {
    ...
    function positionLabels() {
      labels.forEach(function(label) {
        ...
        var leftSideOpacity = 1;
        if (x < 0) { // Left side of planet
          var leftDistance = Math.abs(x) / radius; // 0 to 1
          leftSideOpacity = Math.max(0, 1 - leftDistance * 1.5); // Fade to 0
        }
        
        var finalOpacity = (0.55 + depth * 0.45) * leftSideOpacity;
        label.style.opacity = String(Math.max(0, finalOpacity).toFixed(2));
